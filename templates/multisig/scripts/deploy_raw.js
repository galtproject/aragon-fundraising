...
const { defaultTokenSymbol, defaultTokenName, mainnet, rinkeby } = require('../../../config');
...
module.exports = async callback => {
  try {
    if (process.argv[4] === 'rpc') {
      ...
      const dai = await TokenMock.new(owner, 100000e18, defaultTokenSymbol, defaultTokenName)
      const ant = await TokenMock.new(owner, 100000e18, 'ANT', 'ANT')
      const template = await FundraisingMultisigTemplate.new(daoFactory, ens, miniMeFactory, aragonId, rinkeby.defaultTokenAddress || dai.address, ant.address, { from: owner })
      console.log(template.address)
