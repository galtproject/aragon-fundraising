...
module.exports = async callback => {
  try {
    if (process.argv[4] === 'rpc') {
      ...
      const defaultToken = await TokenMock.new(owner, 100000e18, defaultTokenSymbol, defaultTokenName)
      // ANT rinkeby faucet https://faucet.aragon.black/
      const ANT = '0x0d5263b7969144a852d58505602f630f9b20239d' // OK
      const template = await FundraisingMultisigTemplate.new(daoFactory, ens, miniMeFactory, aragonId, rinkeby.defaultTokenAddress || defaultToken.address, ANT, { from: owner })
      console.log(template.address)
